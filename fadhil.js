...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'FN:√fadhil√\n' // Nama Lu
            + 'ORG:#OwnerGanz;\n' // Nama Kontak Owner
            + 'TEL;type=CELL;type=VOICE;waid=628877558686:+62 887-7558-686\n' // Nomer Lu cok
            + 'END:VCARD'
prefix = "#"
blocked = []
...
async function starts() {
	const fadhil = new WAConnection()
	fadhil.logger.level = 'warn'
	fadhil.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color(' Scan the qr code above'))
	})

	fs.existsSync('./fadhil.json') && fadhil.loadAuthInfo('./fadhil.json')
	fadhil.on('connecting', () => {
		start('2', 'Connecting...')
	})
	fadhil.on('open', () => {
		success('2', 'Connected')
	})
	await fadhil.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./fadhil.json', JSON.stringify(fadhil.base64EncodedAuthInfo(), null, '\t'))

	fadhil.on('group-participants-update', async (anu) => {
		if (!welkom.includes(anu.jid)) return
		try {
			const mdata = await fadhil.groupMetadata(anu.jid)
			console.log(anu)
			if (anu.action == 'add') {
				num = anu.participants[0]
				try {
					ppimg = await fadhil.getProfilePicture(`${anu.participants[0].split('@')[0]}@c.us`)
				} catch {
				...
				fadhil.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			} else if (anu.action == 'remove') {
				num = anu.participants[0]
				try {
					ppimg = await fadhil.getProfilePicture(`${num.split('@')[0]}@c.us`)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
	...
	fadhil.on('chat-update', async (mek) => {
		try {
			...
			const botNumber = fadhil.user.jid
			const ownerNumber = [ownerNumbers]
			const nomorOwner = [ownerNumbers]
			const isGroup = from.endsWith('@g.us')
			const totalchat = await fadhil.chats.all()
			const sender = isGroup ? mek.participant : mek.key.remoteJid
			const groupMetadata = isGroup ? await fadhil.groupMetadata(from) : ''
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        pushname = fadhil.contacts[sender] != undefined ? fadhil.contacts[sender].vname || fadhil.contacts[sender].notify : undefined

			const isUrl = (url) => {
			    return url.match(new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&/=]*)/, 'gi'))
			}
			const reply = (teks) => {
				fadhil.sendMessage(from, teks, text, {quoted:mek})
			}
			const sendMess = (hehe, teks) => {
				fadhil.sendMessage(hehe, teks, text)
			}
			const mentions = (teks, memberr, id) => {
				(id == null || id == undefined || id == false) ? fadhil.sendMessage(from, teks.trim(), extendedText, {contextInfo: {"mentionedJid": memberr}}) : fadhil.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": memberr}})
			}
                        const sendImage = (teks) => {
		                fadhil.sendMessage(from, teks, image, {quoted:mek})
		        }
		        const costum = (pesan, tipe, target, target2) => {
			        fadhil.sendMessage(from, pesan, tipe, {quoted: { key: { fromMe: false, participant: `${target}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target2}` }}})
			}
		        const sendPtt = (teks) => {
		                fadhil.sendMessage(from, audio, mp3, {quoted:mek})
		        }
          ...
          const checkLimit = (sender) => {
                let found = false
                    for (let lmt of _limit) {
                        if (lmt.id === sender) {
                            ...
                            if (limitCounts <= 0) return fadhil.sendMessage(from,`Limit request anda sudah habis\n\n_Note : Limit akan direset setiap jam 21:00!_`, text,{ quoted: mek})
                            fadhil.sendMessage(from, ind.limitcount(limitCounts), text, { quoted : mek})
                            found = true
                        }
                    }
                    if (found === false) {
           ...
           const isLimit = (sender) =>{
              ...
              if (limits >= limitawal ) {
                  position = true
                    fadhil.sendMessage(from, ind.limitend(pushname), text, {quoted: mek})
                    return true
              } else {
                _limit
                  position = true
		...
		case 'info':
			...
			teks = `⟩➢ *Nama Bot* : ${me.name}\n⟩➢ *Nomer Bot* : @${me.jid.split('@')[0]}\n⟩➢ *prefix* : | ${prefix} |\n⟩➢ *Aktif Sejak* : ${kyun(uptime)}\n\n⟩➢ *Instagram* : https://www.instagram.com/mhd_fadhil112\n⟩➢ *Special Thanks To* :\n⟩➢ Allah SWT \n⟩NazwaS\n⟩fadhil`
			buffer = await getBuffer(me.imgUrl)
			fadhil.sendMessage(from, buffer, image, {quoted: mek, caption: teks, contextInfo:{mentionedJid: [me.jid]}})
			break 
                case 'bahasa':
		fadhil.sendMessage(from, bahasa(prefix, sender), text, {quoted: mek})
                break
               case 'virtex':
               fadhil.sendMessage(from, virtex(prefix, sender), text, {quoted: mek})
               break
               case 'kodenegara':
               fadhil.sendMessage(from, negara(prefix, sender), text, {quoted: mek})
               break
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						fadhil.groupDemoteAdmin(from, mentioned)
					} else {
					...
					break
                                case 'randomhentai':
                                        ...
                                        fadhil.sendMessage(from, buffer, image, {quoted: mek})
                  ...
                  case 'kicktime':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					setTimeout( () => {
					fadhil.sendMessage(from, 'Yok Sama" Al-fatihah Buat Yang Keluar', text)
					}, 31000)
					setTimeout( () => {
					reply('Done Min:v')
					}, 30000)
					setTimeout( () => {
					fadhil.groupRemove(from, mentioned)
					}, 29000)
					setTimeout( () => {
					fadhil.sendMessage(from, `Bismilah Kick @${mentioned[0].split('@')[0]} aowkwowk`, text) // ur cods
					}, 24000)
					setTimeout( () => {
					fadhil.sendMessage(from, 'Gw Kick Skrg Ya min', text)
					}, 18000)
					setTimeout( () => {
					fadhil.sendMessage(from, 'Kick Skrg Min? Apa kasih Waktu Lagi?', text)
                    }, 14000)
					setTimeout( () => {
					fadhil.sendMessage(from, `Dah Siap Blm Ngab @${mentioned[0].split('@')[0]}`, text)
					}, 8000)
					setTimeout( () => {
					fadhil.sendMessage(from, 'Otw Kick Min Tapi Gw Kasih Waktu Dlu Ya Min', text)
					}, 2500)
					setTimeout( () => {
					...
					break
                  case 'promote':
					fadhil.updatePresence(from, Presence.composing) 
                                        if (!isRegister) return reply(mess.only.daftarB)
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					...
					if (mentioned.length > 1) {
						...
						mentions(teks, mentioned, true)
						fadhil.groupMakeAdmin(from, mentioned)
					} else {
  ...
  fadhil.updatePresence(from, Presence.composing) 
    ...
    fadhil.sendMessage(from, options, text, { quoted: mek } )
				break
			...
			case 'quotes':
                ...
                if (!isRegister) return reply(mess.only.daftarB)
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                fadhil.sendMessage(from, data, image, {quoted: mek, caption: body.slice(8)})
                await limitAdd(sender)
                break
                case 'fml':
              ...
              case 'owner':
                case 'creator':
                  fadhil.sendMessage(from, {displayname: "Jeff", vcard: vcard}, MessageType.contact, { quoted: mek})
               fadhil.sendMessage(from, 'Nih nomor ownerku kak, save ya kak nanti di save balik',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                fadhil.updatePresence(from, Presence.composing) 
                if (!isRegister) return reply(mess.only.daftarB)
                if (!isGroup) return reply(mess.only.group)
                teks = body.slice(9)
                group = await fadhil.groupMetadata(from);
              ...
              await fadhil.sendMessage(from, options, text)
               break
                                case 'tiktokstalk':
					try {
						if (args.length < 1) return fadhil.sendMessage(from, 'Usernamenya mana kak? ', text, {quoted: mek})
                                                if (!isRegister) return reply(mess.only.daftarB)
                                                if (isLimit(sender)) return reply(ind.limitend(pusname))
						...
						fadhil.sendMessage(from, buffer, image, {quoted: mek, caption: teks})
                                                await limitAdd(sender)
					} catch (e) {
		...
		case 'infonomor':
               fadhil.updatePresence(from, Presence.composing) 
                ...
                break
                   ...
                   fadhil.sendMessage(from, hasil, image, {quoted: mek, caption: `Hasil Dari *${body.slice(5)}*`})
                   await limitAdd(sender)
                   break
                   case 'covidcountry':
                   fadhil.updatePresence(from, Presence.composing) 
				...
				case 'ytmp3':
					...
					fadhil.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result)
					fadhil.sendMessage(from, buffer, audio, {mimetype: 'audio/mp4', filename: `${anu.title}.mp3`, quoted: mek})
                                        await limitAdd(sender)
					break
				case 'ytmp4':
					if (args.length < 1) return reply('Urlnya mana um?')
					...
					fadhil.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result)
					fadhil.sendMessage(from, buffer, video, {mimetype: 'video/mp4', filename: `${anu.title}.mp4`, quoted: mek})
                                        await limitAdd(sender)
					break
				case 'trendtwit':
					fadhil.updatePresence(from, Presence.composing) 
				...
				case 'testime':
					setTimeout( () => {
					fadhil.sendMessage(from, 'Waktu habis:v', text) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
					fadhil.sendMessage(from, '5 Detik lagi', text) // ur cods
					}, 5000) // 1000 = 1s,
					setTimeout( () => {
					fadhil.sendMessage(from, '10 Detik lagi', text) // ur cods
					}, 0) // 1000 = 1s,
					break
				/*case 'semoji':
					...
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						...
						fadhil.sendMessage(from, buffer, sticker)
						fs.unlinkSync(rano)
					})
					break*/
				case 'nulis':
				case 'tulis':
					...
					fadhil.sendMessage(from, buff, image, {quoted: mek, caption: mess.success})
                                        await limitAdd(sender)
					break
	case 'kbbi':
            fadhil.updatePresence(from, Presence.composing) 
	      ...
	      tels = body.slice(6)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
                ...
                fadhil.sendMessage(from, buffer, image, {quoted: mek, caption: infomp3})
                fadhil.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${data.result.title}.mp3`, quoted: mek})
                await limitAdd(sender)
                break
				case 'blocklist':
					...
					fadhil.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": blocked}})
					break
                   case 'chatlist':
					fadhil.updatePresence(from, Presence.composing)  
					...
					fadhil.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": totalchat}})
					break
				case 'animecry':
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
					...
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						...
						if (err) return reply(mess.error.stick)
						buffer = fs.readFileSync(rano)
						fadhil.sendMessage(from, buffer, sticker, {quoted: mek})
						fs.unlinkSync(rano)
					})
					break
				case 'neonime':
                ...
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                fadhil.sendMessage(from, data, image, {quoted: mek, caption: body.slice(7)})
                await limitAdd(sender)
                break
				case 'tts':
				   fadhil.updatePresence(from, Presence.recording) 
				   if (args.length < 1) return fadhil.sendMessage(from, 'Kode bahasanya mana om?', text, {quoted: mek})
                                   if (!isRegister) return reply(mess.only.daftarB)
                                   if (isLimit(sender)) return reply(ind.limitend(pusname))
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return fadhil.sendMessage(from, 'Textnya mana om', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
					...
					: gtts.save(ranm, dtt, function() {
						exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
							...
							fadhil.sendMessage(from, buff, audio, {quoted: mek, ptt:true})
							fs.unlinkSync(rano)
						})
					})
                                        await limitAdd(sender)
				...
				case 'pokemon':
					...
					fadhil.sendMessage(from, pok, image, { quoted: mek })
                                        await limitAdd(sender)
					break
                case 'pinterest':
                                        tels = body.slice(11)
					...
					nimek =  n[Math.floor(Math.random() * n.length)];
					pok = await getBuffer(nimek)
					fadhil.sendMessage(from, pok, image, { quoted: mek, caption: `*PINTEREST*\n\*Hasil Pencarian* : *${tels}*`})
                                        await limitAdd(sender)
					break
				case 'setprefix':
					fadhil.updatePresence(from, Presence.composing) 
				...
				case 'meme':
					...
					fadhil.sendMessage(from, buffer, image, {quoted: mek, caption: '.......'})
                                        await limitAdd(sender)
					break
				case 'memeindo':
                                        if (!isRegister) return reply(mess.only.daftarB)
				...
				case 'block':
					...
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
					fadhil.blockUser (`${body.slice(8)}@c.us`, "add")
					fadhil.sendMessage(from, `perintah Diterima, memblokir ${body.slice(8)}@c.us`, text)
					break
				case 'hilih':
					fadhil.updatePresence(from, Presence.composing) 
				...
				case 'send':
					...
					fadhil.sendMessage(nomor+'@s.whatsapp.net', pesan, text)
					break
					case 'quotesnime':
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				if (!isQuotedImage) return reply(`Kirim gambar dengan caption ${prefix}setbotpp atau tag gambar yang sudah dikirim`)
					...
					media = await fadhil.downloadAndSaveMediaMessage(enmedia)
					await fadhil.updateProfilePicture(botNumber, media)
				...
				case 'bc':
					...
					anu = await fadhil.chats.all()
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						buff = await fadhil.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							fadhil.sendMessage(_.jid, buff, image, {caption: `*『 BROADCAST 』*\n\n${body.slice(4)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of groupMembers) {
							fadhil.sendMessage(_.jid, buff, image, {caption: `*『 BC GROUP 』*\n*Group* : ${groupName}\n\n${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of groupMembers) {
				...
				case 'alay':
                    ...
                    fadhil.sendMessage(from, buffer, image, {quoted: mek, caption: 'neh...'})
                    await limitAdd(sender)
                    break
		case 'tahta':
                if (!isRegister) return reply(mess.only.daftarB)
                ...
                tahta = `${body.slice(7)}`
                     ...
                     fadhil.sendMessage(from, buff, image, {quoted: mek, caption: `Harta Tahta ${tahta}`})
                  await limitAdd(sender) 
                  break  
                    case 'glitch':
                    gh = body.slice(7)
                    ...
                    fadhil.sendMessage(from, hasil, image, {quoted: mek, caption: 'neh...'})
                    await limitAdd(sender)
                    break
                     case 'leave':
                    if (!isGroup) return reply(mess.only.group)
                    if (!isGroupAdmins) return reply(mess.only.admin)
                     setTimeout( () => {
					fadhil.groupLeave (from) 
					}, 2000)
                     setTimeout( () => {
					fadhil.updatePresence(from, Presence.composing) 
					fadhil.sendMessage(from, 'Sayonara👋', text) // ur cods
					}, 0)
                     break
			...
			case 'igstalk':
                    ...
                    fadhil.sendMessage(from, buffer, image, {quoted: mek, caption: hasil})
                    await limitAdd(sender)
                    break
                    case 'ownergrup':
				  case 'ownergroup':
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           case 'quran':
					...
					fadhil.sendMessage(from, quran, text, {quoted: mek})
					break
           case 'nekonime':
           ...
           fadhil.sendMessage(from, hasil, image, {quoted: mek})
           await limitAdd(sender)
           break
				case 'neko':
                                        if (!isRegister) return reply(mess.only.daftarB)
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						fadhil.groupRemove(from, mentioned)
					fadhil.sendMessage(mentioned, 'yahaha Lu kekick😂', text)
					}
					break
				case 'exe':
	              fadhil.updatePresence(from, Presence.composing) 
	              if (!isOwner) return reply(mess.only.ownerB)
	               ...
	               exec(cmd, (err, stdout) => {
		           if(err) return fadhil.sendMessage(from, "Command Salah", text, { quoted: mek })
		           if (stdout) {
			       fadhil.sendMessage(from, stdout, text, { quoted: mek })
		           }
	           })
                  break
                 case 'linkgroup':
				...
				case 'linkgc':
				    ...
				    if (!isGroup) return reply(mess.only.group)
                                     if (!isRegister) return reply(mess.only.daftarB)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					linkgc = await fadhil.groupInviteCode (from)
					yeh = `https://chat.whatsapp.com/${linkgc}\n\nLink Group *${groupName}*`
					fadhil.sendMessage(from, yeh, text, {quoted: mek, detectLinks: false})
					break
                case 'qrcode':
                if (!isRegister) return reply(mess.only.daftarB)
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                buff = await getBuffer(`https://api.qrserver.com/v1/create-qr-code/?data=${body.slice(8)}&size=1080%C3%971080`)
				fadhil.sendMessage(from, buff, image, {quoted: mek})
                                await limitAdd(sender)
				break
				case 'ocr':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						const media = await fadhil.downloadAndSaveMediaMessage(encmedia)
						reply(mess.wait)
					...
					break
                     ...
                     const bug = body.slice(5)
                      if (pesan.length > 300) return fadhil.sendMessage(from, 'Maaf Teks Terlalu Panjang, Maksimal 300 Teks', msgType.text, {quoted: mek})
                    ...
                    fadhil.sendMessage(NomerOwner, options, text, {quoted: mek})
	    ...
	    case 'kapankah':
			...
			case 'closegc':
					...
					fadhil.groupSettingChange (from, GroupSettingChange.messageSend, true);
					reply(close)
					break
                case 'opengc':
                case 'bukagc':
					...
					fadhil.groupSettingChange (from, GroupSettingChange.messageSend, false)
					fadhil.sendMessage(from, open, text, {quoted: mek})
					break
				case 'stiker':
				case 'sticker':
				case 'stickergif':
				case 'stikergif':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Finish')
								fadhil.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
								fs.unlinkSync(media)
								fs.unlinkSync(ran)
							})
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								...
								fadhil.sendMessage(from, buff, sticker)
								fs.unlinkSync(media)
								fs.unlinkSync(ran)
							})
							.addOutputOptions([`-vcodec`,`libwebp`,`-vf`,`scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15, pad=320:320:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`])
				...
				case 'toimg':
					...
					media = await fadhil.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.png')
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						buffer = fs.readFileSync(ran)
						fadhil.sendMessage(from, buffer, audio, {mimetype: 'audio/mp4', quoted: mek})
						fs.unlinkSync(ran)
					})
					break
	          ...
	          if (!isRegister) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
                ...
                fadhil.sendMessage(from, buffer, image, {quoted: mek, caption: infomp3})
                fadhil.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${anu.title}.mp3`, quoted: mek})
                await limitAdd(sender)
                break
                     case 'infocuaca':
                   tels = body.slice(11)
                   ...
                   fadhil.sendMessage(from, hasil, text, {quoted: mek})
                   await limitAdd(sender)
                   break
                              case 'game':
					...
					setTimeout( () => {
					fadhil.sendMessage(from, '*➣ Jawaban :* '+anu.result.jawaban+'\n'+anu.result.desk, text, {quoted: mek}) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					fadhil.sendMessage(from, '_10 Detik lagiâ€¦_', text) // ur cods
					}, 20000) // 1000 = 1s,
					setTimeout( () => {
					fadhil.sendMessage(from, '_20 Detik lagi_â€¦', text) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
					fadhil.sendMessage(from, '_30 Detik lagi_â€¦', text) // ur cods
					}, 1000) // 1000 = 1s,
					setTimeout( () => {
					fadhil.sendMessage(from, anu.result.soal, text, { quoted: mek }) // ur cods
					}, 0) // 1000 = 1s,
					break
                                case 'welcome':
					if (!isGroup) return reply(mess.only.group)
					...
					fadhil.sendMessage(from, 'fakta : '+ keh, { quoted: mek })
                                        await limitAdd(sender)
					break
                                case 'water':
				...
				case 'firetext':
					...
					fadhil.sendMessage(from, 'Pertanyaan : *'+ganteng+'*\n\nJawaban : '+ teng+'%', text, { quoted: mek })
					break
					case 'cantikcek':
					if (isRegister) return reply(mess.only.daftarB)
					cantik = body.slice(1)
					...
					fadhil.sendMessage(from, 'Pertanyaan : *'+cantik+'*\n\nJawaban : '+ tik+'%', text, { quoted: mek })
					break
				case 'watak':
				if (isRegister) return reply(mess.only.daftarB)
                                if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					fadhil.sendMessage(from, 'Pertanyaan : *'+watak+'*\n\nJawaban : '+ tak, text, { quoted: mek })
                                        await limitAdd(sender)
				    break
				case 'hobby':
				if (isRegister) return reply(mess.only.daftarB)
					...
					fadhil.sendMessage(from, 'Pertanyaan : *'+hobby+'*\n\nJawaban : '+ by, text, { quoted: mek })
					break
                                case 'nsfwneko':
				    try{
						...
						fadhil.sendMessage(from, buffer, image, {quoted: mek, caption: 'mesum'})
                                                await limitAdd(sender)
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❰❗❱ *ERROR* ❰❗❱')
				    ...
				    try{
						...
						buffer = await getBuffer(res.result)
                                                if (!isRegister) return reply(mess.only.daftarB)
                                                if (isLimit(sender)) return reply(ind.limitend(pusname))
						fadhil.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nich'})
                                                await limitAdd(sender)
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❰❗❱ *ERROR* ❰❗❱')
				...
				case 'logowolf2':
					...
					if (!isGroup)return reply(mess.only.group)
                                        if (!isRegister) return reply(mess.only.daftarB)
					if (!isGroupAdmins)return reply(mess.only.admin)
					fadhil.deleteMessage(from, { id: mek.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true })
					break
                                case 'phlogo':
					var gh = body.slice(7)
					var gbl1 = gh.split("|")[0];
                ...
                case 'truth':
					...
					fadhil.sendMessage(from, truteh, image, { caption: '*Truth*\n\n'+ ttrth, quoted: mek })
					break
                                case 'dare':
					...
					fadhil.sendMessage(from, tod, image, { quoted: mek, caption: '*Dare*\n\n'+ der })
					break	
                case 'level':
               ...
               fadhil.sendMessage(from, resul, text, { quoted: mek})
                .catch(async (err) => {
                        console.error(err)
                        await reply(`Error!\n${err}`)
                    })
            break
				...
				mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					...
					var bot = gh.split("|")[2];
					fadhil.sendMessage(from, `${bot}`, text, {quoted: { key: { fromMe: false, participant: `${mentioned}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target}` }}})
					break
            case 'leveling':
                if (!isGroup) return reply(mess.only.group)
                if (!isGroupAdmins) return reply(mess.only.admin)
            ...
            break
                                ...
                                case 'nsfwtrap':
                                        try{
                                                ...
                                                buffer = await getBuffer(res.result)
                                                fadhil.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih gambarnya kak...'})
                                                await limitAdd(sender)
                                        } catch (e) {
                                                console.log(`*Error* :`, color(e,'red'))
                                                reply('❰❗❱ *ERROR* ❰❗❱')
			   	        ...
			   	        if (!isRegister) return reply(mess.only.userB)
                                        ...
                                        fadhil.updatePresence(from, Presence.composing) 
				        uptime = process.uptime()
                                        fadhil.sendMessage(from, `Speed: *${latensi.toFixed(4)} _Second_*\nDevice: *Black Shark 3*\nRAM: *8/128*\nData: *Telkomsel*\nJaringan: *4G*\nStatus: *Gak Di Cass*`, text, { quoted: mek})
                                        break
                                case 'neonlogo':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        fadhil.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih kak logonya...'})
                                        await limitAdd(sender)
                                        break
                                case 'neonlogo2':
                                ...
                                case 'jsholat':
                                        ...
                                        mbteks = `「 *JAM SHALAT* 」\n ➣ Daerah : ${loc} \n ➣ Ashar : ${anu.Ashar} \n ➣ Dhuha : ${anu.Dhuha} \n ➣ Dzuhur : ${anu.Dzuhur} \n ➣ Imsyak : ${anu.Imsyak} \n ➣ Isya : ${anu.Isya} \n ➣ Maghrib : ${anu.Maghrib} \n ➣ Subuh : ${anu.Subuh}`
                                        fadhil.sendMessage(from, mbteks, text)
                                        break
                                case 'jokerlogo':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
			       ...
			       reply(anu.result.jadwalTV)
					break*/
                                case 'afk':
                                        ...
                                        fadhil.sendMessage(from, tag, text, {quoted: mek})
                                        break
                                case 'shadow':
                                        var gh = body.slice(7)
                                        var teks1 = gh.split("|")[0];
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        fadhil.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih kak gambarnya...'})
                                        await limitAdd(sender)
                                        break
                                case 'burnpaper':
                                        var gh = body.slice(10)
                                ...
                                case 'randomkpop':
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        fadhil.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih kpopnya kak...'})
                                        await limitAdd(sender)
                                        break
                                case 'fml2':
					...
					if (anu.error) return reply(anu.error)
					buffer = await getBuffer(anu.result)
					fadhil.sendMessage(from, buffer, video, {quoted: mek})
                                        await limitAdd(sender)
					break
				case 'ttp':
					...
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(ranp)
						if (err) return reply(mess.error.stick)
						fadhil.sendMessage(from, fs.readFileSync(rano), sticker, {quoted: mek})
						fs.unlinkSync(rano)
					})
                                        await limitAdd(sender)
					break
					...
					anu = await fadhil.chats.all()
					fadhil.setMaxListeners(25)
					for (let _ of anu) {
						fadhil.deleteChat(_.jid)
					}
				...
				case 'clone':
					...
					try {
						pp = await fadhil.getProfilePicture(id)
						buffer = await getBuffer(pp)
						fadhil.updateProfilePicture(botNumber, buffer)
					...
					break
                                case 'pubglogo':
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        fadhil.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih logonya kak...'})
                                        await limitAdd(sender)
                                        break
                                case 'herrypotter':
                                case 'harrypotter':
			                ...
			                if (!isBotGroupAdmins) return reply(ind.badmin())
                                        fadhil.groupUpdateSubject(from, `${body.slice(9)}`)
                                        fadhil.sendMessage(from, 'Succes, Ganti Nama Grup', text, {quoted: mek})
				        break
                                case 'setdesc':
                                        if (!isGroup) return reply(ind.groupo())
			                if (!isGroupAdmins) return reply(ind.admin())
			         	if (!isBotGroupAdmins) return reply(ind.badmin())
                                        fadhil.groupUpdateDescription(from, `${body.slice(9)}`)
                                        fadhil.sendMessage(from, 'Succes, Ganti Deskripsi Grup', text, {quoted: mek})
					break*/
                                case 'translate':
                                        aruga = body.slice(10)
                                        lang = aruga.split("|")[0];
                                        ...
                                        fadhil.sendMessage(from, arteks, text)
                                        await limitAdd(sender)
                                        break
                                case 'tafsirmimpi':
                                        aruga = body.slice(12)
                                ...
                                case 'tagme':
                                        ...
                                        fadhil.sendMessage(from, tagme, text, {quoted: mek})
                                        break
                                case 'ip': // masih testing
                                        ...
                                        fadhil.sendMessage(from, teks, text)
                                        await limitAdd(sender)
                                        break
                                case 'happymod':
                                        toby = body.slice(10)
                                ...
                                case 'say':
                                        ...
                                        saying = teks
                                        fadhil.sendMessage(from, saying, text)
                                        break
			        case 'wait':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                                        if (!isRegister) return reply(mess.only.daftarB)
						reply(mess.wait)
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						media = await fadhil.downloadMediaMessage(encmedia)
						await wait(media).then(res => {
         						fadhil.sendMessage(from, res.video, video, {quoted: mek, caption: res.teks.trim()})
						}).catch(err => {
							reply(err)
		      				})
					} else {
